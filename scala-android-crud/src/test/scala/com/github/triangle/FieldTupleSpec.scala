...
class FieldTupleSpec extends Spec with ShouldMatchers {
    ...
    val tuple = new FieldTuple3(intField, stringField, doubleField) {
      ...
      Unit match {
        case Values(integer, string, double) => {
          integer should be (Some(10))
          string should be (Some("Hello"))
          double should be (Some(11.0))
        }
      }
  ...
  it("should work for tuple size of 5") {
  ...
  it("should work for tuple size of 6") {
  ...
  it("should work for tuple size of 7") {
